...
class BooksController extends OCSController {
	...
	public function addUserBook(string $title, string $author = ''): DataResponse {
		try {
			$book = $this->bookMapper->createBook($this->userId, $title, $author);
	...
	public function editUserBook(int $id, ?string $title = null, ?string $author = null): DataResponse {
		try {
			$book = $this->bookMapper->updateBook($id, $this->userId, $title, $author);
			return new DataResponse($book);
		} catch (Exception|Throwable $e) {
			return new DataResponse(['error' => $e->getMessage()], Http::STATUS_BAD_REQUEST);
		}
